...
## Задача1: Повышение безопасности системы
...
## Задача2: Разработка сервиса отчётов
...
## Задача3: Снижение нагрузки на базу данных
...
- добавлен клиент для работы с S3 ([s3.go](/task3/reports-backend/internal/storage/s3.go)), который поддерживает все необходимые функции работы с бакетами, публикации и загрузки файлов.
- добавлен лёгкий метод GetPeriodByUserID (только period_start, period_end) для построения ключа в S3 без полного запроса к витрине ([repository.go](/task3/reports-backend/internal/repository/repository.go)). В перспективе определение периода построения отчёта лучше вынести в пользовательский UI, чтобы полностью исключить запросы к БД.
- добавлен handler для получения готового отчёта ([server.go](/task3/reports-backend/internal/server/server.go)). 
...
В storage ([s3.go](/task3/reports-backend/internal/storage/s3.go)) добавлен `EnsureBucketLifecycle` - правило `lifecycle` для префикса reports/ с `Expiration` — N дней. Количество дней задаётся через env `S3_REPORT_TTL_DAYS` (по умолчанию 7); при старте бэкенда правило выставляется в Minio.

### Фронтенд

Во фронтенде ([ReportPage.tsx](/task3/frontend/src/components/ReportPage.tsx)) изменена страница с отчётом: по кнопке "Получить отчёт" отображается кнопка "Открыть отчёт в новой вкладке" и iframe с отчётом по этой ссылке.
...
## Задача 4: Повышение оперативности и стабильности работы CRM
